...
do
    ...
    if [ $is_scale ]; then
        ...
        kubectl scale deployment orders-edge --replicas=$(( $(kubectl get deployment carts-edge -n cloud-sock-shop -o=jsonpath='{.spec.replicas}') + 1)) -n cloud-sock-shop
    fi
	sleep 120
    ...
    if [ $is_scale ]; then
        ...
        kubectl scale deployment carts-edge --replicas=$(( $(kubectl get deployment carts-edge -n cloud-sock-shop -o=jsonpath='{.spec.replicas}') - 1)) -n cloud-sock-shop
    fi
