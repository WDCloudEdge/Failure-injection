...
do
    ...
    if [ $is_scale ]; then
        ...
        kubectl scale deployment recommendationservice --replicas=$(( $(kubectl get deployment recommendationservice -n hipster -o=jsonpath='{.spec.replicas}') + 1)) -n hipster
    fi
	sleep 120
    ...
    if [ $is_scale ]; then
        ...
        kubectl scale deployment productcatalogservice --replicas=$(( $(kubectl get deployment productcatalogservice -n hipster -o=jsonpath='{.spec.replicas}') - 1)) -n hipster
    fi
