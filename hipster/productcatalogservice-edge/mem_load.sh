...
do
    ...
    if [ $is_scale ]; then
        ...
        kubectl scale deployment productcatalogservic-edge --replicas=$(( $(kubectl get deployment productcatalogservice-edge -n hipster -o=jsonpath='{.spec.replicas}') - 1)) -n hipster
    fi
