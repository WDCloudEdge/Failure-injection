...
do
    ...
    if [ $is_scale ]; then
        ...
        kubectl scale deployment recommendationservice-edge --replicas=$(( $(kubectl get deployment recommendationservice-edge -n hipster -o=jsonpath='{.spec.replicas}') + 1)) -n hipster
    fi
	sleep 120
    ...
    if [ $is_scale ]; then
        ...
        kubectl scale deployment productcatalogservice-edge --replicas=$(( $(kubectl get deployment productcatalogservice-edge -n hipster -o=jsonpath='{.spec.replicas}') - 1)) -n hipster
    fi
